...
if (isset($_GET['tid'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM `oldtenants` WHERE oldtenantid = ?");
...
<body>
    <div class="flex flex-col text-center justify-center min-h-screen gap-1">
        ...
        <div>
            <Label>Doc 1 : <?php echo " " . $tdoc1; ?></Label>
            <button class="text-blue-700 font-semibold underline hover:text-blue-900">View</button>
        </div>
        <div>
            <Label>Doc 2 :<?php echo " " . $tdoc2; ?></Label>
            <button class="text-blue-700 font-semibold underline hover:text-blue-900">View</button>
        </div>
