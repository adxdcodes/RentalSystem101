...
$sql = "SELECT * FROM oldtenants LIMIT $limit OFFSET $offset";
...
<body class="flex flex-col min-h-screen select-none">
  ...
  <div class="flex justify-center flex-1 bg-gray-800 text-black">
    <div class="container mx-auto p-8">
      <h1 class="text-3xl font-bold mb-6 text-white"><a href="old-tenants.php"> Old Tenants</a></h1>
      <!--- ********************** search bar *************************** -->
            ...
            <?php foreach ($tenants as $tenant) : ?>
              <tr class="text-center <?php echo $counter % 2 == 0 ? 'bg-gray-100' : 'bg-white'; ?>">
                ...
                <td class="py-2 px-4 border-b">
                  <a href="admin-show-old-details.php?tid=<?php echo htmlspecialchars($tenant['oldtenantid']); ?>" target="_blank" class="bg-yellow-500 text-black cursor-pointer py-1 mx-1 px-3 rounded hover:bg-yellow-600 transition duration-300">
                    Details
                  </a>

                  <?php
                  // Assume $tenant['tenant_id'] is numeric and $tenant['tenant_name'] is a string
                  echo '<button onclick="openModal(' . htmlspecialchars($tenant['oldtenantid']) . ', \'' . addslashes(htmlspecialchars($tenant['tenant_name'])) . '\')" class="bg-red-500 text-black py-1 cursor-pointer px-3 rounded hover:bg-red-600 transition duration-300">Delete</button>';
                  ?>
                </td>
              </tr>
            <?php endforeach; ?>
      ...
      $total_result = mysqli_query($conn, "SELECT COUNT(*) AS count FROM oldtenants");
  ...
  <script>
    $(document).ready(function() {
      $(' #search_text').on('keyup', function() {
        ...
        if (txt === '') {
          $.ajax({
            url: "fetch-all-old-tenants.php", // Create a new PHP file to handle fetching all tenants
            method: "get",
            dataType: "html",
